...
### Perps signals workflow (`perps-signals/`)

Runs on a **cron schedule** (e.g. every 60 seconds): fetches the latest price from the Marshmallow backend (`GET /api/perps/price`), then POSTs a signal to the backend (`POST /api/perps/cre-signal`). Used for perps: read Data Streams/Feeds → compute signals/risk → output for execution. The backend must be running and have the price/signal endpoints (and optional `CRE_WEBHOOK_SECRET`) configured.

**How it connects:** You do **not** trigger it from the runtime dashboard. Once deployed, the cron runs automatically on schedule on Chainlink’s DON. The dashboard (cre.chain.link) is for monitoring and activating/pausing. See [docs/PERPS_CRE_FLOW.md](docs/PERPS_CRE_FLOW.md) for the full flow (simulate → deploy → production).

**Run perps-signals (local simulation):**
1. Start the Marshmallow backend (e.g. `cd backend && npm run dev`).
2. In `perps-signals/config.json` set `backendBaseUrl` to your backend URL (e.g. `http://localhost:3001`).
3. `cd cre-workflow && bun install` (or `npm install`) in the workflow dir if needed.
4. From `cre-workflow/`: `cre workflow simulate perps-signals --target local-simulation`. Log in with `cre login` if prompted. When the menu appears, select the cron trigger (or use `--non-interactive --trigger-index 0`).

## Project Structure

```
cre-workflow/
├── prediction-market-demo/      # Prediction market settlement workflow
│   ├── main.ts                  # Log trigger → Gemini → settle → Firestore
...
├── perps-signals/               # Perps signals workflow (this project)
│   ├── main.ts                  # Cron trigger → fetch price → POST signal to backend
│   ├── http.ts                  # HTTP GET /api/perps/price, POST /api/perps/cre-signal
│   ├── types.ts                 # Config and types
│   ├── config.json              # backendBaseUrl, optional creWebhookSecret
│   ├── workflow.yaml            # CRE workflow settings
│   └── package.json             # Dependencies

...
function getPriceRequest(sendRequester, config) {
  const url = `${config.backendBaseUrl.replace(/\/$/, "")}/api/perps/price`;
  const req = { url, method: "GET", cacheSettings: { readFromCache: true, maxAgeMs: 1e4 } };
  const resp = sendRequester.sendRequest(req).result();
  const bodyText = new TextDecoder().decode(resp.body);
  if (!ok(resp))
    throw new Error(`GET price failed: ${resp.statusCode} ${bodyText}`);
  return JSON.parse(bodyText);
}
function postSignalRequest(body) {
  return (sendRequester, config) => {
    const url = `${config.backendBaseUrl.replace(/\/$/, "")}/api/perps/cre-signal`;
    const headers = { "Content-Type": "application/json" };
    if (config.creWebhookSecret)
      headers["X-CRE-Secret"] = config.creWebhookSecret;
    const req = {
      url,
      method: "POST",
    ...
    const bodyText = new TextDecoder().decode(resp.body);
    if (!ok(resp))
      throw new Error(`POST cre-signal failed: ${resp.statusCode} ${bodyText}`);
    return { ok: true };
  };
}
function fetchPrice(runtime2) {
  const httpClient = new cre.capabilities.HTTPClient;
  return httpClient.sendRequest(runtime2, getPriceRequest, consensusIdenticalAggregation())(runtime2.config).result();
}
function sendSignal(runtime2, body) {
  const httpClient = new cre.capabilities.HTTPClient;
  httpClient.sendRequest(runtime2, postSignalRequest(body), consensusIdenticalAggregation())(runtime2.config).result();
}
var onCronTrigger = (runtime2, payload) => {
  if (payload?.scheduledExecutionTime) {
    const { seconds } = payload.scheduledExecutionTime;
    runtime2.log(`Perps signals run at ${String(seconds)}`);
  }
  const priceResult = fetchPrice(runtime2);
  runtime2.log(`Price: ${priceResult.price} updatedAt: ${priceResult.updatedAt}`);
  ...
  sendSignal(runtime2, body);
  runtime2.log(`Signal sent: ${signal}`);

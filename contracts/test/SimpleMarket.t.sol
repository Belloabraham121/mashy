...
contract SimpleMarketTest is Test {
    ...
    // Short duration for test markets (3 minutes, matching original behavior)
    uint256 internal constant TEST_DURATION = 3 minutes;
    ...
    function test_newMarket_initializesFields() public {
        uint256 id = market.newMarket("Will ETH close above $2000?", TEST_DURATION);
        assertEq(id, 0);
        ...
        assertEq(close, open + TEST_DURATION);
    }
    ...
    function test_makePrediction_updatesTotalsAndCounts() public {
        uint256 id = market.newMarket("Q", TEST_DURATION);
    ...
    function test_makePrediction_reverts_afterClose() public {
        uint256 id = market.newMarket("Q", TEST_DURATION);
        vm.warp(block.timestamp + TEST_DURATION + 1); // move past close time
    ...
    function _prepareAndRequestSettlement(string memory question) internal returns (uint256 id) {
        id = market.newMarket(question, TEST_DURATION);
        vm.warp(block.timestamp + TEST_DURATION + 1);
        market.requestSettlement(id);
    }
